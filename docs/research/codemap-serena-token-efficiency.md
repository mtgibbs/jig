...
**Related:** `pi-cluster/docs/research/local-coding-agent-sdd.md` (the qwen/opencode harness this is meant to serve)

> Imported verbatim from pi-cluster (`docs/research/`) on 2026-08-30, because this repo's
> README leans on the 20–56% figure below and a stranger must be able to check it without
> access to the private repo the trials ran in. Cross-repo citations inside are qualified
> with a `pi-cluster/` prefix; the trials' raw output (`results/results.jsonl`) stays there.
...
`pi-cluster/.claude/skills/coding-agent-ops/SKILL.md`). Round 3: audit 0/90 wrong-rooted.
