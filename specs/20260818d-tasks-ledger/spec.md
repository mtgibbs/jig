...
- **Touches:** new `scripts/ralph-ledger.sh`, `scripts/ralph-build.sh`, `scripts/ralph-codex.sh`,
  new `specs/20260818d-tasks-ledger/{tasks.txt,verify.sh,fixtures/}`.
...
- `specs/20260818b-loop-doctor` classifies harness faults but has **no cross-run corpus** to classify
...
- Cross-run analysis, corpus building, or trend reporting. The ledger is the *substrate* those
  need; consuming it belongs to `specs/20260818b-loop-doctor` and a future reader.
- Reordering, dependency resolution, or parallel task execution.
- Any change to the three-verdict contract, STRICT, or the reset.
- Merging with the run-scoped state in `specs/20260818c-run-regression-guard` — see §6.
...
- **`ralph-build.sh` and `ralph-codex.sh` are deliberate structural twins.** Every change lands in
  both, identically. `specs/20260818c-run-regression-guard` AC11 asserts this by comparing extracted lines.
- **`specs/20260818c-run-regression-guard` introduces a *different* state file** —
...
`specs/20260818d-tasks-ledger/verify.sh`, hermetic, two tiers — copy the shape of
`specs/20260818c-run-regression-guard/verify.sh`, and **create every directory the loop is pointed at** (the
...
`scripts/run-loop.sh build-converge specs/20260818d-tasks-ledger` from a worktree on a throwaway branch.
**Same caution as `specs/20260818a-ralph-retry-contract` and `specs/20260818c-run-regression-guard`: the loop being
