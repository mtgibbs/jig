#!/usr/bin/env bash
# verify.sh — deterministic acceptance gate for specs/20260818d-tasks-ledger.
# Run from repo root:  ./specs/20260818d-tasks-ledger/verify.sh   (STRICT=1 for the final pass)
#
...
FIX="specs/20260818d-tasks-ledger/fixtures"
ROOT_ABS="$(pwd)"
fail=0
ok(){   echo "  PASS  $1"; }
...
stray="$(find specs/20260818d-tasks-ledger -type f \
  ! -name spec.md ! -name tasks.txt ! -name verify.sh \
  ! -path 'specs/20260818d-tasks-ledger/fixtures/*' \
  ! -path 'specs/20260818d-tasks-ledger/evidence/*' 2>/dev/null)"
[ -z "$stray" ] && ok "scope:no-litter-in-spec-dir" \
  || { no "scope:no-litter-in-spec-dir"; echo "$stray" | sed 's/^/          /' >&2; }

BASE="${TL_BASE:-HEAD}"   # HEAD, not origin/main: the guard asks what THIS gate run is blessing (the working tree), not what the whole branch did — issue #33/spec 20260831k
if git rev-parse --verify --quiet "$BASE" >/dev/null 2>&1; then
  git diff --quiet "$BASE" -- specs/TEMPLATE.md scripts/gate-score.sh scripts/loop-report.sh \
      ...
      specs/20260818b-loop-doctor specs/20260818c-run-regression-guard 2>/dev/null \
...
if [ ! -f "$L" ] || ! grep -q 'ledger_record' "$Q" 2>/dev/null; then
  ...
  # AC13 (twin symmetry) removed by specs/20260825c-executor-binding — see run-regression-guard.
fi
