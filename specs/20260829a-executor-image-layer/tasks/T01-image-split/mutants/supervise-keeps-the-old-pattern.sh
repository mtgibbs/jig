...
# WHY: the one file a renamer forgets. The binding moved, ralph-build.sh's default moved, the
# WHY: three landed gates moved — and the supervisor's process-pattern list still matches the
# WHY: old basename, so it stops recognising a running loop. Nothing errors: it reports no loop
# WHY: running, which is also what it reports when no loop is running.
#!/usr/bin/env bash
