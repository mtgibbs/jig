...
# WHY: adds the announcement and hangs it on the FLAG rather than on node. It fires when a
# WHY: human passed RALPH_SHEET=off — the case nobody needed telling about — and stays silent
# WHY: in the only case that matters: the sheet is wanted, node is absent, and the run quietly
# WHY: loses 20-56% of its measured context saving with nothing in the log to say so.
#!/usr/bin/env bash
