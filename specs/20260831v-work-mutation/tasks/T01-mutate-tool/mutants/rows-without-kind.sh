...
# WHY: the kind=work marker is dropped from every row — telemetry becomes
# WHY: indistinguishable from the enforcement record, which is the one confusion the
# WHY: whole vocabulary split exists to prevent.
#!/usr/bin/env bash
