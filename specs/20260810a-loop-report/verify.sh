#!/usr/bin/env bash
# verify.sh — deterministic acceptance gate for specs/20260810a-loop-report.
# Run from repo root:  ./specs/20260810a-loop-report/verify.sh
# Hermetic: fixtures only; never invokes gate-score.sh (recursion hazard, spec §5).
set -uo pipefail
F="scripts/loop-report.sh"
FIX="specs/20260810a-loop-report/fixtures"
fail=0
ok(){ echo "  PASS  $1"; }
...
OUT="$(run --spec specs/20260810a-loop-report --base HEAD --judge-state "$FIX" --gate-log "$FIX/gate.log" || true)"
...
OUT2="$(run --spec specs/20260810a-loop-report --base HEAD --judge-state "$FIX" --gate-log specs/20260810a-loop-report/spec.md || true)"
echo "$OUT2" | grep -qx 'gate: none' && ok "gate:none-without-sentinel" || no "gate:none-without-sentinel"

# AC5 — empty judge-state.
EMPTY="$(mktemp -d)"
OUT3="$(run --spec specs/20260810a-loop-report --base HEAD --judge-state "$EMPTY" || true)"
rmdir "$EMPTY"
