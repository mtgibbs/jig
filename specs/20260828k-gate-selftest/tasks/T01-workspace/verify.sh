...
bad=""
( cd "$T/r" && timeout 30 bash "$ST" ) >"$T/e0" 2>&1; [ $? = 0 ] && bad="$bad no-arg-accepted"
( cd "$T/r" && timeout 30 bash "$ST" specs/fx/tasks/NOPE ) >"$T/e1" 2>&1; [ $? = 0 ] && bad="$bad missing-dir-accepted"
mkdir -p "$T/r/specs/fx/tasks/T99-a/mutants"
( cd "$T/r" && timeout 30 bash "$ST" specs/fx/tasks/T99-a ) >"$T/e2" 2>&1; [ $? = 0 ] && bad="$bad no-verify-accepted"
mkdir -p "$T/r/specs/fx/tasks/T98-b"; cp "$T/r/specs/fx/tasks/T01-thing/verify.sh" "$T/r/specs/fx/tasks/T98-b/"
( cd "$T/r" && timeout 30 bash "$ST" specs/fx/tasks/T98-b ) >"$T/e3" 2>&1; [ $? = 0 ] && bad="$bad no-mutants-accepted"
