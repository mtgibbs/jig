#!/usr/bin/env bash
# verify.sh — deterministic acceptance gate for specs/20260801a-scored-gate.
...
#
# Run from repo root:  ./specs/20260801a-scored-gate/verify.sh
set -uo pipefail
...
echo "VERIFY specs/20260801a-scored-gate  ($GS)"
...
outside="$(git status --porcelain 2>/dev/null | awk '{print $2}' | grep -vE '^(scripts/gate-score\.sh|specs/20260801a-scored-gate/)' || true)"
if [ -n "$outside" ]; then
