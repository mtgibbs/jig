#!/usr/bin/env bash
# specs/20260825b-evidence-spec-nesting/verify.sh — acceptance gate for the slug-nested evidence layout.
# Run from repo root:  bash specs/20260825b-evidence-spec-nesting/verify.sh   (STRICT=1 for the final pass)
#
...
# not loop behaviours. specs/20260818a-ralph-retry-contract and specs/20260818c-run-regression-guard already drive
...
# ---------------------------------------------------------------- scope / litter (FIRST)
stray="$(find specs/20260825b-evidence-spec-nesting -type f \
  ! -name spec.md ! -name tasks.txt ! -name verify.sh \
  ! -path 'specs/20260825b-evidence-spec-nesting/fixtures/*' \
  ! -path 'specs/20260825b-evidence-spec-nesting/evidence/*' 2>/dev/null)"
...
#      (specs/20260825c-executor-binding); a check naming a pair goes stale the moment the roster
...
for f in spec.md tasks.txt verify.sh; do
  [ -f "specs/20260825b-evidence-spec-nesting/$f" ] && ok "convention-self-applies-$f" presence \
    || no "convention-self-applies-$f" "missing" presence
done
gl=$(wc -l < specs/20260825b-evidence-spec-nesting/verify.sh)
