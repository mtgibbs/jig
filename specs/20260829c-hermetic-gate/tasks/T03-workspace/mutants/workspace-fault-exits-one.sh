...
# WHY: turns a workspace fault into exit 1. Callers read 2 as "could not run" and 1 as "the work
# WHY: is wrong", so an unusable mount is filed in the run evidence as a failed assertion and the
# WHY: loop spends its retries on a container problem.
