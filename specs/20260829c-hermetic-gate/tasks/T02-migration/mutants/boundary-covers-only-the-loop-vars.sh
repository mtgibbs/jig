...
# WHY: clears the loop control variables at the boundary and not the reporting ones — the
# WHY: skipping bug fixed, the coordinator leak left. 23 gates source nothing, so for them this
# WHY: is the only reset there is.
#!/usr/bin/env bash
