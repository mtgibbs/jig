...
- **Touches:** `scripts/ralph-retry.sh`, `scripts/ralph-build.sh`, `scripts/ralph-codex.sh`,
  new `specs/20260818c-run-regression-guard/{tasks.txt,verify.sh,fixtures/}`.
- **Source:** run `qwen-10668` on `specs/20260818b-loop-doctor`, 2026-08-18 — six tasks passed
...
- `specs/20260818c-run-regression-guard/{tasks.txt,verify.sh,fixtures/}`.
...
- `specs/20260818b-loop-doctor/**` — its T6 defect is the evidence for this spec, not its subject.
...
- **Testing a loop is solved here**: `specs/20260818a-ralph-retry-contract/verify.sh` runs the real
  ...
  its own AC required (`specs/20260818a-ralph-retry-contract/evidence/`).
- **Redirect the side channels in tests**: `RALPH_STATUS_DIR`, `RALPH_LOG_DIR` under a temp dir
  ...
  which `specs/20260818b-loop-doctor` reads.
...
`specs/20260818c-run-regression-guard/verify.sh`, hermetic, two tiers — copy the shape of
`specs/20260818a-ralph-retry-contract/verify.sh` and **create every directory the loop is pointed at**
(§6).
...
`scripts/run-loop.sh build-converge specs/20260818c-run-regression-guard` from a worktree on a throwaway
branch. **The same §11b caution as `specs/20260818a-ralph-retry-contract` applies: the loop being modified
