#!/usr/bin/env bash
# specs/20260825c-executor-binding/verify.sh — acceptance gate for the build-phase executor binding.
# Run from repo root:  bash specs/20260825c-executor-binding/verify.sh   (STRICT=1 for the final pass)
#
...
# ---------------------------------------------------------------- scope / litter
stray="$(find specs/20260825c-executor-binding -type f ! -name spec.md ! -name tasks.txt ! -name verify.sh \
  ! -path 'specs/20260825c-executor-binding/evidence/*' 2>/dev/null)"
...
refs="$(grep -rln 'ralph-codex' specs/*/verify.sh scripts/ 2>/dev/null \
        | grep -v '^specs/20260825c-executor-binding/' | head -5)"
...
tw="$(grep -rln 'twins-do-not-drift\|twins-identical' specs/*/verify.sh 2>/dev/null \
      | grep -v '^specs/20260825c-executor-binding/' | head -3)"
...
for f in spec.md tasks.txt verify.sh; do
  [ -f "specs/20260825c-executor-binding/$f" ] && ok "convention-self-applies-$f" presence \
    || no "convention-self-applies-$f" "missing" presence
done
gl=$(wc -l < specs/20260825c-executor-binding/verify.sh)
