  ...
  if [ ! -d "$d/tasks" ]; then
    ...
    while [ "$i" -le "$n" ]; do
      ...
      if [ -z "$td" ]; then
        ...
        # The corpus requirement (20260831u). --check judges NEW work (header above), so
        # this is unconditional — the date boundary lives in the LOOP, the half that meets
        # history. The scaffold's sentinel template counts here: --check is the authoring
        # tool and authoring is in progress; the LOOP is what refuses to run a sentinel.
        _mok=0
