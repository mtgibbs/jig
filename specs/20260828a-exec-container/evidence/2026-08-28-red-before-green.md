# Red before green — `specs/20260828a-exec-container`
...
```
$ bash        specs/20260828a-exec-container/verify.sh ; echo $?   → 0   (10 pend, 2 PASS, 0 FAIL)
$ STRICT=1 bash specs/20260828a-exec-container/verify.sh ; echo $? → 1
```
...
Four presence guards, four stubs — the rule from `specs/20260827b-fleet-run-key`, where two unopened guards
...
final task forever under `specs/20260827c-last-task-strict`, and a gate that cannot pass is a broken control
rather than a strict one — the lesson from
`specs/20260826a-evidence-replayable/evidence/2026-08-27-ac8-control-stale-path.md`.
...
"One stub per presence guard" (`specs/20260827b-fleet-run-key`) is necessary and not sufficient. The stub
