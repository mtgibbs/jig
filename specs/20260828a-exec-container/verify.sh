#!/usr/bin/env bash
# specs/20260828a-exec-container/verify.sh — the deterministic gate for the containerised binding.
#
...
# forever under specs/20260827c-last-task-strict, and a gate that cannot pass is a broken control).
#
...
if [ ! -f "$BIND" ]; then
  ...
  if [ "${ARGC:-0}" -lt 2 ]; then
    ...
    # Thinness, per specs/20260825c-executor-binding §3/§7. Comments stripped so a sentence about a gate
    # is not a gate.
    _body="$(sed 's/#.*//' "$BIND")"
