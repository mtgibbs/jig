#!/usr/bin/env bash
# verify.sh — deterministic acceptance gate for specs/20260818a-ralph-retry-contract.
# Run from repo root:  ./specs/20260818a-ralph-retry-contract/verify.sh   (STRICT=1 for the final pass)
#
...
# corpus (which specs/20260818b-loop-doctor reads) and never reaches the network.
#
...
# specs/20260802a-judge-loop/verify.sh drives the real ralph-judge.sh against mock commands.
#
...
FIX="specs/20260818a-ralph-retry-contract/fixtures"
...
stray="$(find specs/20260818a-ralph-retry-contract -type f \
  ! -name spec.md ! -name tasks.txt ! -name verify.sh \
  ! -path 'specs/20260818a-ralph-retry-contract/fixtures/*' \
  ! -path 'specs/20260818a-ralph-retry-contract/evidence/*' 2>/dev/null)"
...
if git rev-parse --verify --quiet "$BASE" >/dev/null 2>&1; then
  git diff --quiet "$BASE" -- scripts/ralph-judge.sh scripts/ralph-status.sh scripts/ralph-log.sh \
      ...
      scripts/loops specs/TEMPLATE.md specs/20260818b-loop-doctor 2>/dev/null \
...
if grep -q 'section 10 acceptance criteria' "$Q" 2>/dev/null; then
  ...
  # ac12 (twin symmetry) removed by specs/20260825c-executor-binding — the twin is gone.
else
...
if grep -q 'ralph-retry.sh' "$Q" 2>/dev/null; then
  ...
  # the same false-positive class as specs/20260818b-loop-doctor's ac15. Scope to the block.
