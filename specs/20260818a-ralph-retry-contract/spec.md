...
- **Touches:** `scripts/ralph-build.sh`, `scripts/ralph-codex.sh`, new `scripts/ralph-retry.sh`,
  new `specs/20260818a-ralph-retry-contract/{tasks.txt,verify.sh,fixtures/}`.
- **Source:** dogfood run `qwen-61568` on `specs/20260818b-loop-doctor`, 2026-08-18 — stopped on T1 after
...
T1 of `specs/20260818b-loop-doctor`, three attempts, one check away from passing the whole time:
...
blind spot the judge loop closed with `total == total_base` (`specs/20260802a-judge-loop/spec.md` §6:
...
(`specs/20260802a-judge-loop/reviews/2026-08-02-codex-planning-check.md`, finding #4):
...
> `specs/20260818b-loop-doctor/evidence/2026-08-18-red-before-green.md` §4).
...
- `specs/20260818a-ralph-retry-contract/{tasks.txt,verify.sh,fixtures/}`.
...
- Any `clusters/pi-k3s/**` manifest, and `specs/20260818b-loop-doctor/**`.
...
- **`clean -fd` must not become `-fdx`.** `opencode.json` is gitignored and MUST survive; without
  ...
  `specs/20260818b-loop-doctor` classifies as `permission-blocked`.
...
- **Testing a loop is a solved problem here.** `specs/20260802a-judge-loop/verify.sh` runs the real
...
- **Redirect the loop's side channels in tests.** `RALPH_STATUS_DIR`, `RALPH_LOG_DIR` and
  ...
  `~/.harness` corpus — which `specs/20260818b-loop-doctor` reads.
...
`specs/20260818a-ralph-retry-contract/verify.sh`, hermetic. Two tiers:
...
AC9, AC13. Precedent: `specs/20260802a-judge-loop/verify.sh` drives the real `ralph-judge.sh` with mock
commands.
...
regression notice. That is measurable only by re-running `specs/20260818b-loop-doctor` after this lands
(§12/OQ1), which is the point of having a corpus.

## 11b. Loop execution

`scripts/run-loop.sh build-converge specs/20260818a-ralph-retry-contract` from a worktree on a throwaway
...
- **OQ1 — Does a regression notice actually help?** Unknown, and measurable: re-run
  `specs/20260818b-loop-doctor` T1 after this lands and compare against `qwen-61568`'s ledger row. If qwen
  ...
  the pair as the first entry in the cross-run corpus `specs/20260818b-loop-doctor` exists to build.
