#!/usr/bin/env bash
# specs/20260826a-evidence-replayable/verify.sh — the deterministic gate for "an attempt's record is
...
_stray="$(find "$R/specs/20260826a-evidence-replayable" -maxdepth 1 -mindepth 1 \
...
mkdir -p "$T/specs/20260826a-evidence-replayable"
...
probe() {
  ...
  ROOT="$REPO" SPEC_DIR="$T/specs/20260826a-evidence-replayable" \
...
if [ "$(id -u 2>/dev/null || echo 1)" = 0 ]; then
  ...
  SPEC_DIR="$T/specs/20260826a-evidence-replayable" RALPH_LOG_DIR="$T/ev" RALPH_AGENT=gate \
  bash "$T/probe.sh" >/dev/null 2>&1
  rc=$?
...
{"agent":"gate","repo":"fixture","spec":"specs/20260826a-evidence-replayable","branch":"b","task_index":1,
