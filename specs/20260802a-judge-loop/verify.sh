#!/usr/bin/env bash
# verify.sh — deterministic acceptance gate for specs/20260802a-judge-loop.
...
#
# Run from repo root:  bash specs/20260802a-judge-loop/verify.sh     (STRICT=1 for the final pass)
set -uo pipefail
...
# INTO the target repo on purpose since specs/20260825b-evidence-spec-nesting (2026-08-24). A bare
...
if ! "$_probe/x" 2>/dev/null; then
  ...
  echo "         TMPDIR, e.g.  TMPDIR=\"\$HOME/tmp\" bash specs/20260802a-judge-loop/verify.sh" >&2
  echo; echo "VERIFY: ENV"; exit 2
fi
rm -rf "$_probe"

echo "VERIFY specs/20260802a-judge-loop  ($RJ)"
...
outside="$(git status --porcelain -- . ':!.evidence' 2>/dev/null | awk '{print $2}' | grep -vE '^(scripts/ralph-judge\.sh|specs/20260802a-judge-loop/)' || true)"
if [ -n "$outside" ]; then
  no "changes outside scripts/ralph-judge.sh + specs/20260802a-judge-loop/: $(echo "$outside" | tr '\n' ' ')(§5 — fixtures go in \$TMPDIR)"
else
