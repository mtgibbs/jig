...
class H(BaseHTTPRequestHandler):
    ...
    def _plain(self, code, raw):
        # text/plain, always. These are bytes a WORKER supplied — a prompt, a patch, a gate
        # transcript, a mutant's diff — and the board fetches them from its own origin. Served
    ...
    def _artifact(self):
        ...
        The run key is <host>/<agent>-<pid> and CONTAINS A SLASH, which is why it rides the
        ...
        # Named separately. The stub this replaces answered "key required" to every request
        # including the ones that had a key, which told a caller nothing about what was wrong.
        if not key:
