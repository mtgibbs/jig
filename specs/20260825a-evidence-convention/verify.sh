#!/usr/bin/env bash
# specs/20260825a-evidence-convention/verify.sh — STATIC gate for the evidence convention.
#
...
for f in scripts/ralph-build.sh; do   # one build loop since specs/20260825c-executor-binding
...
if [ -f specs/20260825a-evidence-convention/tasks.txt ]; then
  ...
  # them is not.
  first_self=$(grep -n 'ralph-build.sh' specs/20260825a-evidence-convention/tasks.txt | head -1 | cut -d: -f1)
  last_other=$(grep -vn 'ralph-build.sh' specs/20260825a-evidence-convention/tasks.txt | grep -c . >/dev/null; grep -n . specs/20260825a-evidence-convention/tasks.txt | grep -v 'ralph-build.sh' | tail -1 | cut -d: -f1)
...
for f in spec.md tasks.txt verify.sh; do
  [ -f "specs/20260825a-evidence-convention/$f" ] && ok "convention-self-applies-$f" presence \
    || no "convention-self-applies-$f" "missing"
done
gl=$(wc -l < specs/20260825a-evidence-convention/verify.sh)
