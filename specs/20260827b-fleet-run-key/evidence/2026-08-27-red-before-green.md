# Red before green — `specs/20260827b-fleet-run-key`

Validated against `05ee8d7` on the container harness, **two ways**: an empty tree, and a stub.

```
$ bash        specs/20260827b-fleet-run-key/verify.sh ; echo $?   → 0   (12 pend, 6 PASS, 0 FAIL)
$ STRICT=1 bash specs/20260827b-fleet-run-key/verify.sh ; echo $? → 1
```
...
`specs/20260826a-evidence-replayable` had a red-before-green pass and still shipped an AC-8 control that
**could not pass for any implementation**; it failed correct work three times before anyone
noticed (`specs/20260826a-evidence-replayable/evidence/2026-08-27-ac8-control-stale-path.md`). The reason
