#!/usr/bin/env bash
# specs/20260827b-fleet-run-key/verify.sh — the deterministic gate for "a run key that survives two workers".
#
...
# specs/20260826a-evidence-replayable shipped broken and failed correct work three times
...
_stray="$(find "$R/specs/20260827b-fleet-run-key" -maxdepth 1 -mindepth 1 \
...
# baseline run (see specs/20260802a-judge-loop/verify.sh). Probed up front instead.
...
mkdir -p "$T/specs/20260827b-fleet-run-key"
...
probe() {
  ...
  ROOT="$REPO" SPEC_DIR="$T/specs/20260827b-fleet-run-key" \
...
if [ -f "$KEY" ]; then
  ...
  # control, not a strict one (specs/20260826a-evidence-replayable/evidence/2026-08-27-ac8-control-stale-path.md).
...
if [ -z "$D" ] || [ ! -d "$D" ]; then
  ...
  # AC-10 of specs/20260826a-evidence-replayable, asserted here as a REGRESSION GUARD: this spec is the
  ...
  # AC-9 of specs/20260826a-evidence-replayable, same reasoning: a regression guard, not an assumption.
...
exit 0
TWO
GATE_LOG_SH="$LOG" GATE_OUT="$T" ROOT="$REPO" SPEC_DIR="$T/specs/20260827b-fleet-run-key" \
...
if grep -q 'run-key' "$STAT" 2>/dev/null; then
  ...
  ( RALPH_STATUS_DIR="$T/st" ROOT="$REPO" SPEC_DIR="$T/specs/20260827b-fleet-run-key" \
