...
- **Tools:** git, jq, sed
- **MCP:** none
- **Permissions:** write:scripts/**, write:specs/20260827b-fleet-run-key/**, exec:git
- **Touches:** `scripts/run-key.sh` (new), `scripts/ralph-log.sh` (`log_init`, the GC),
  ...
  `scripts/loop-index.py` (the readers), and one line of `specs/20260827a-spec-manifest/verify.sh` (§5).
...
2. The run directory's **leaf is still exactly `<agent>-<pid>`** — every reader parses the pid out
   of it, and `specs/20260826a-evidence-replayable` AC-10 asserts it. This spec does not get to change that.
3. `latest` keeps working exactly as `specs/20260826a-evidence-replayable` AC-9 specifies: a slash-free
...
Added to the per-attempt `<stem>.json` of `specs/20260826a-evidence-replayable` §3.2. Both are **strings**,
...
full of dashes. `specs/20260826a-evidence-replayable` AC-10 asserts the leaf matches `<agent>-<pid>`, and
...
`specs/20260827a-spec-manifest/verify.sh` forbade a `Tools:` declaration in **every** spec but its own. The
