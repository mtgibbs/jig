...
# WHY: strips one path segment too many, so 'would run:' prints
# WHY: '<slug>/tasks/<task>' without the specs/ prefix. qwen's actual attempt-1
# WHY: bug in both watched runs — the gate must keep discriminating it.
#!/usr/bin/env bash
