...
- **Touches:** new `scripts/loop-doctor.sh`; new `specs/20260818b-loop-doctor/{tasks.txt,verify.sh,fixtures/}`.
...
- `specs/20260818b-loop-doctor/{tasks.txt,verify.sh,fixtures/}` — the gate and its fixtures.
...
- **Style + gate idiom to mirror:** `specs/20260810a-loop-report/verify.sh` — hermetic, fixtures-only,
...
`specs/20260818b-loop-doctor/verify.sh`, **hermetic — fixtures only**, never invoking `gate-score.sh` or
any loop (§4). Mirrors `specs/20260810a-loop-report/verify.sh`: `ok()`/`no()`/`pend()` per the three-verdict
...
Fixtures to ship under `specs/20260818b-loop-doctor/fixtures/`, each a **minimal reproduction of a real
...
check cannot pass vacuously. Record the red-before evidence in `specs/20260818b-loop-doctor/evidence/`.
...
Standard: `scripts/run-loop.sh build-converge specs/20260818b-loop-doctor` from a git worktree on a
