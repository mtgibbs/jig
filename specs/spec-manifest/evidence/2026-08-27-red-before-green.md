# Red before green — and green proven reachable — for `specs/spec-manifest`

Run against `8fa87e3` with **no task built**, from `~/dev/harness-manifest`.

```
$ bash specs/spec-manifest/verify.sh   ; echo $?      → 0   (14 pend · 5 PASS · 0 FAIL)
$ STRICT=1 bash specs/spec-manifest/verify.sh; echo $? → 1
```
...
`git status` shows only `specs/spec-manifest/` untracked.
