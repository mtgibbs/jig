...
- **Tools:** git, sed
- **MCP:** none
- **Permissions:** write:scripts/spec-field.sh, write:scripts/run-loop.sh, write:specs/TEMPLATE.md, write:specs/README.md, write:specs/spec-manifest/**
...
| the executor is a binding, and its config file differs per binding | `specs/executor-binding` §3 | the `MCP:` check must derive the config path from `RALPH_EXEC_CMD`, never hardcode `opencode.json` |
...
`specs/spec-manifest/verify.sh`. Functional: it builds fixture `spec.md` files in a scratch dir
